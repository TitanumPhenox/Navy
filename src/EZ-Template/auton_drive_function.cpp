...
void drive_pid_task(void *) {
    ...
    while (true) {
        ...
        if (DEBUG && pros::millis() > 1500) {
            if (active_drive_type == drive)
                printf("le: %f   re: %f   l_der %f\n", left_error, right_error, l_der * drive_constant[direction][1]);
            if (active_drive_type == turn || active_drive_type == l_swing || active_drive_type == r_swing)
                printf("output: %f   error: %f   p: %f   i: %f   d: %f\n", gyro_output, gyro_error,
                       gyro_error * gyro_kp, gyro_integral * gyro_ki, gyro_der * gyro_kd);
        }
...
void set_drive_pid(int type, float target, int speed, bool slew_on, bool toggle_heading) {
    // Global setup
    ...
    if (type == drive) {
        ...
        l_slope =
                (SLEW_MIN_POWER[direction] - max_speed) / ((l_start + (SLEW_DISTANCE[direction] * TICK_PER_INCH)) - 0);
        r_slope =
                (SLEW_MIN_POWER[direction] - max_speed) / ((l_start + (SLEW_DISTANCE[direction] * TICK_PER_INCH)) - 0);
    }
...
bool
drive_exit_condition(int small_timeout, int start_small_counter_within, int big_timeout, int start_big_counter_within,
                     int velocity_timeout) {
    ...
    if (fabs(l_target_encoder - left_sensor()) < start_small_counter_within &&
        fabs(r_target_encoder - right_sensor()) < start_small_counter_within) {
        j++;
        // printf("\nJ: %i", j/10);
    ...
    if (fabs(l_target_encoder - left_sensor()) < start_big_counter_within &&
        fabs(r_target_encoder - right_sensor()) < start_big_counter_within) {
        i++;
        // printf("\nI: %i", i/10);
...
bool
turn_exit_condition(int small_timeout, int start_small_counter_within, int big_timeout, int start_big_counter_within,
                    int velocity_timeout) {
    static int i, j, k;
    static int delay_time = 10;
...
void skills_auton() {
    ...
    pros::delay(500);
    set_motors_drive(0);
    while (gyro.get_heading() < 91 || gyro.get_heading() > 96) {
